	...
	if(mysqli_connect_errno()){
		...
		if (isset($_SESSION['adminuser'])){	
			if(isset($_POST['stud_id'])){
				...
				$dup = mysqli_query($con, "select * from tor where stud_id = '" . $stud_id . "'");
				if(mysqli_fetch_row($dup) == 0){
					mysqli_query($con, "insert into tor values('$stud_id', '', '')");
				}				
			}
			else{
				header("location:tor.php");
			}
		}
		else{
...
<h1 style="margin-bottom: -7vh"> <?php echo $_POST['name'] ?>'s TOR </h1><br>
  <div class="back">
		<?php
			$back = "window.location='tor.php'";
...
<div class="container mt-5">
    <div class="row">
		<div class="col-sm-8">
		<?php 
			$sql = mysqli_query($con, "select * from tor where stud_id = '" . $stud_id . "'");
			$data = mysqli_fetch_array($sql);
			$filepat = "tor/tor" . $_POST['stud_id'] .".pdf";
			echo $filepat;

			if(file_exists($filepat)){
				mysqli_query($con, "update tor set torfile = '" . $filepat . "' where stud_id = '" . $stud_id . "'");
		...
		<form action="upload_file.php" method="POST" enctype="multipart/form-data">
			...
			<input type="hidden" name = "filepat" value = "<?php echo "tor/tor" . $_POST['stud_id'] . ".pdf";?>">
		...
		<form action="access.php" method="POST">
			...
			<input type="hidden" name = "stud_id" value = "<?php echo $_POST['stud_id'];?>">
			<input type="hidden" name = "name" value = "<?php echo $_POST['name'];?>">
			<input type="radio" name="acs" value="1" <?php if($data['toraccess'] == 1){ echo "checked"; }?>><label> YES </label>
			<input type="radio" name="acs" value="0" <?php if($data['toraccess'] == 0){ echo "checked"; }?>><label> NO </label>
