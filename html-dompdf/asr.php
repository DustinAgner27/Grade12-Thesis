...
<div class="container-fluid p-5 text-center bg-202225">
  <h1 style="margin-bottom: -7vh">ASRS </h1>
  <div class="back">
		<?php
			echo '<form><input style="text-decoration: bold; text-align: left; color: white; background-color: #2A2D33; border-color: #2A2D33;width: 2vw; height: 5vh;" type="button" value="<" onClick="javascript:hisasry.go(-1)"></form>';
			?>
		</div>
</div>
    ...
    <div class="row">
		...
		<?php 
			if($res2 != null){
				...
				foreach($ar as $bat){
					echo "<div class = 'col-sm-2''><div style='background-color:#ff9499;color:black; height: auto; width: 99%; border-radius: 10px; box-shadow: 0 0 20px 2px black; box-shadow: 0 7px 20px 0.1px black;text-align: center;'><h1  style='margin-top:20px; text-shadow: 0 0 0 black;'> B " . $ar[$batchc][0][4] . "</h1>";
								foreach ($bat as $data){		
									$ctr++;				
										echo "<form action = 'studentasr.php' method = 'POST' >";
										echo "<input type='hidden' name = 'stud_id' value = '" . $data[0] . "'>";
										echo "<input type='hidden' name = 'name' value = '" . $data[1] .  ", " . $data[2] . "'>";
										echo "<input style='width: 90%; height: 50px; border-radius: 10px; margin-bottom: 0; background-color: #302529;border: 0; box-shadow: 0 7px 20px 0.1px black;' type='submit' value = '" . $data[1] . ", " . $data[2][0] . "'>";
										echo "</form>";
								}
								$batchc++;
								echo "<br></div></div>";
