		...
		if (isset($_SESSION['user'])){	
			...
			$dup = mysqli_query($con, "select * from reportcard where stud_id = '" . $stud_id . "'");
				if(mysqli_fetch_row($dup) == 0){
					mysqli_query($con, "insert into reportcard values('$stud_id', '', '')");
				}				
...
<h1 style="margin-bottom: -7vh"> <?php echo $user['lname'] . ", " . $user['fname']?>'s REPORT CARD </h1>
  <div class="back">
		<?php
    ...
    <div class="row">
		<div class="col-sm-12">
			<?php 
				...
				$filepat = $data['repcardfile'];
				if(file_exists($filepat) && $data['repcardaccess'] == 1){
